...
def test_fiat_live_profile_exposes_extended_gasoline_obd_sensors():
    ...
    assert {
        ...
        0x2E, 0x3C, 0x41, 0x42, 0x44, 0x45, 0x47, 0x48, 0x49, 0x4A,
        0x4B, 0x4C, 0x50, 0x52, 0x53, 0x54, 0x5A, 0x5D, 0x5F, 0x61,
        0x62, 0x63,
    } <= definitions.keys()
    ...
    assert PID_BY_ID[0x3C].decoder(bytes.fromhex("1388")) == 460
    assert PID_BY_ID[0x61].decoder(bytes.fromhex("7D")) == 0
    assert PID_BY_ID[0x63].decoder(bytes.fromhex("012C")) == 300
