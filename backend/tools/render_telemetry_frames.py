...
def draw_overlay(frame: Any, state: dict[str, Any]) -> None:
    ...
    panel(frame, margin, margin, 650, 285)
    ...
    text(frame, f"EAT6 {transmission_text(state)}", margin + 390, margin + 62, 0.82, CYAN, 2)
    text(frame, elapsed_text(float(state.get("capture_elapsed_s") or 0)), margin + 20, margin + 96, 0.55)
    ...
    for row, (label, color) in enumerate(lines):
        text(frame, label, margin + 20, margin + 128 + row * 27, 0.63, color, 1)

    right_width = 620
    right_x = width - margin - right_width
    panel(frame, right_x, margin, right_width, 265)
    ...
    right_lines = (
        ...
        (f"LKA etat     {value_text(state.get('lka_state'), 0)}", WHITE),
        (f"LKA couple   {value_text(state.get('lka_torque_command_raw'), 0)} brut", WHITE),
