...
Toutes les commandes de ce fichier restent en observation véhicule : elles
n'envoient aucune trame CAN et aucune consigne de direction. Le capteur Matek,
s'il est activé, reçoit uniquement des requêtes de lecture MSP sur son USB.
Dans les quatre
cas, la vidéo brute, l'overlay openpilot et les prédictions sont enregistrés
sur le Mac dans
`data/runtime/openpilot_live`. Avec la
...
```bash
./openpilot/scripts/record_peugeot_lka.sh --list-cameras
```
...
Le guide complet de câblage, d'orientation et de validation se trouve dans
[docs/MATEK_F722_SE_SENSORS.md](docs/MATEK_F722_SE_SENSORS.md).
...
```bash
../openpilot/.venv/bin/python openpilot/tools/run_openpilot_live.py \
...
- `can.jsonl` : trames CAN brutes synchronisées, uniquement dans les cas 2 et
  4.
- `sensors.jsonl` : IMU, attitude et GPS auxiliaires horodatés lorsque la Matek
