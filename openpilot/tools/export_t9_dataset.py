...
SUPPORT_FILES = (
  "openpilot/scripts/build_t9_lateral_on_device.py",
  "openpilot/scripts/fetch_comma_308_diagnostics.sh",
  "openpilot/scripts/install_t9_lateral_on_device.py",
...
def _code_files(repo_root: Path) -> list[tuple[str, bytes]]:
  port = repo_root / "openpilot/port/t9_lateral"
  if not (port / "base-sha256.json").is_file():
    raise ValueError(f"T9 source tree missing below {repo_root}")
  files = []
  for path in sorted(port.rglob("*")):
    ...
    files.append((f"code/{relative.as_posix()}", path.read_bytes()))
