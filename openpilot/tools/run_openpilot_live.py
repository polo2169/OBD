...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sensor-port", help="Port du capteur GPS/IMU auxiliaire")
    parser.add_argument("--sensor-baud", type=int, default=921600)
    parser.add_argument(
        "--sensor-protocol",
        choices=("esp32", "matek-msp"),
        default="esp32",
        help="Protocole du capteur auxiliaire (ESP32 texte ou Matek MSP)",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if args.sensor_port and not (args.record or args.record_overlay):
        raise ValueError("--sensor-port nécessite --record afin de créer sensors.jsonl")
    if args.sensor_port and args.port and args.sensor_port == args.port:
        raise ValueError("Les ports CAN et GPS/IMU doivent être différents")
    ...
    if args.record or args.record_overlay:
        recorder = LiveRecorder(
            ...
            args.record_overlay,
            {
                ...
                "sensor_baud": args.sensor_baud if args.sensor_port else None,
                "sensor_protocol": args.sensor_protocol if args.sensor_port else None,
                "sensor_mount_yaw_deg": (
                    args.sensor_mount_yaw_deg
                    if args.sensor_port and args.sensor_protocol == "matek-msp" else None
                ),
    ...
    try:
        ...
        if args.sensor_port:
            ...
            sensor_tool = Path(__file__).with_name(
                "record_matek_sensors.py"
                if args.sensor_protocol == "matek-msp"
                else "record_esp32_sensors.py"
            )
            if not sensor_tool.is_file():
                raise FileNotFoundError(sensor_tool)
            sensor_command = [
                executable_path(args.backend_python), str(sensor_tool),
                "--port", args.sensor_port, "--baud", str(args.sensor_baud),
                "--output", str(recorder.directory / "sensors.jsonl"),
            ]
            if args.sensor_protocol == "matek-msp":
                sensor_command += ["--mount-yaw-deg", str(args.sensor_mount_yaw_deg)]
            ...
            if sensor_process.poll() is not None:
                raise RuntimeError(
                    f"L'enregistreur GPS/IMU {args.sensor_protocol} s'est arrêté "
                    f"(code {sensor_process.returncode})"
                )
        source: int | Path = (
            args.stream.resolve()
            if args.stream is not None
        ...
        if sensor_process is not None and sensor_process.poll() is None:
            ...
            try:
                sensor_process.wait(timeout=3.0)
            except subprocess.TimeoutExpired:
                sensor_process.kill()
                sensor_process.wait(timeout=1.0)
        if not args.headless:
