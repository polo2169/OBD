...
RUNNER="${LAB_DIR}/tools/run_openpilot_live.py"
...
sensor_port="${ESP32_SENSOR_PORT:-}"
sensor_protocol="${LKA_SENSOR_PROTOCOL:-esp32}"
sensor_mount_yaw_deg="${MATEK_MOUNT_YAW_DEG:-0}"
duration_s="900"
...
Usage:
  openpilot/scripts/record_peugeot_lka.sh [options]

Options:
  --port CHEMIN       Port de l'ESP32 véhicule (préférence mémorisée par défaut)
  --sensor-port PATH  Port du GPS/IMU auxiliaire
  --sensor-protocol P esp32 ou matek-msp (esp32 par défaut)
  --sensor-mount-yaw DEG
                      Flèche Matek à 0/90/180/270° horaire depuis l'avant
...
if [[ "${sensor_protocol}" != "esp32" && "${sensor_protocol}" != "matek-msp" ]]; then
  echo "--sensor-protocol doit valoir esp32 ou matek-msp." >&2
  exit 2
fi
if [[ ! "${sensor_mount_yaw_deg}" =~ ^(0|90|180|270)$ ]]; then
  echo "--sensor-mount-yaw doit valoir 0, 90, 180 ou 270." >&2
  exit 2
fi
...
if [[ -n "${sensor_port}" ]]; then
  if [[ ! -e "${sensor_port}" ]]; then
    echo "Port GPS/IMU introuvable: ${sensor_port}" >&2
    exit 1
  fi
  if [[ "${sensor_port}" == "${can_port}" ]]; then
    echo "Les ports CAN et GPS/IMU doivent être différents." >&2
    exit 2
  fi
  if command -v lsof >/dev/null 2>&1 && lsof "${sensor_port}" >/dev/null 2>&1; then
    echo "Le port GPS/IMU est déjà utilisé: ${sensor_port}" >&2
    exit 1
  fi
fi
...
if [[ -n "${sensor_port}" ]]; then
  sensor_baud=921600
  if [[ "${sensor_protocol}" == "matek-msp" ]]; then
    sensor_baud=115200
  fi
  command+=(
    --sensor-port "${sensor_port}"
    --sensor-protocol "${sensor_protocol}"
    --sensor-baud "${sensor_baud}"
    --sensor-mount-yaw-deg "${sensor_mount_yaw_deg}"
  )
fi
...
echo "  GPS/IMU: ${sensor_port:-désactivé} (${sensor_protocol})"
...
if sys.argv[2]:
    sensor_counts = {}
    with (session / "sensors.jsonl").open(encoding="utf-8") as stream:
        for line in stream:
            if not line.strip():
                continue
            try:
            ...
            sensor_counts[kind] = sensor_counts.get(kind, 0) + 1
    if sys.argv[3] == "matek-msp":
        if sensor_counts.get("imu", 0) < 10 or sensor_counts.get("gps", 0) < 1:
            raise SystemExit(
                "Capture Matek incomplète: "
                f"IMU={sensor_counts.get('imu', 0)}, GPS={sensor_counts.get('gps', 0)}"
            )
    elif sum(sensor_counts.values()) < 10:
        raise SystemExit("Capture GPS/IMU auxiliaire vide ou trop courte")
...
if sys.argv[2]:
    print(f"  GPS/IMU {sys.argv[3]}: {sensor_counts}")
' "${latest_session}" "${sensor_port}" "${sensor_protocol}"
