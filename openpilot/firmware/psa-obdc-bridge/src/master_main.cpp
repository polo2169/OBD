...
bool torque_command_seen = false;
uint32_t last_torque_command_ms = 0;
bool mads_gate_raw = false;
bool mads_gate_enabled = false;
uint32_t mads_gate_changed_ms = 0;
...
void emit_hello() {
  ...
  doc["mads"] = true;
  doc["mads_unified_engagement"] = false;
  doc["mads_brake_mode"] = "disengage_rearm_required";
  doc["mads_physical_enable"] = "active_low";
  doc["mads_enable_gpio"] = static_cast<int>(bridge_cfg::MADS_ENABLE_PIN);
  doc["can0"] = "eps";
  doc["can2"] = "car_bsi";
  doc["state"] = state_name();
  emit_json(doc);
...
void emit_stats() {
  ...
  doc["mads_state"] = safety.mads_state_name();
  doc["mads_requested"] = safety.mads_requested();
  doc["mads_engaged"] = safety.mads_engaged();
  doc["mads_rearm_required"] = safety.mads_rearm_required();
  doc["mads_physical_enable"] = safety.physical_mads_enable();
  doc["mads_reason"] = safety.mads_reason();
  ...
  doc["torque_command_age_ms"] = torque_command_seen
      ? millis() - last_torque_command_ms : 0xFFFFFFFFU;
    ...
    case link_protocol::Type::CanFrame: {
      ...
      if (!extended && !remote && packet.identifier == psa_rvv_safety::ID_RVV) {
        ...
        if (action == psa_rvv_safety::FrameAction::Reject) {
          const char *reason = rvv.reason();
          emit_event("RVV_DISABLED_LATERAL_REMAINS", reason);
          rvv.set_command(false, psa_rvv_safety::MIN_SETPOINT_KPH, now);
          memcpy(rvv_payload, packet.data, packet.length);
        }
        payload = rvv_payload;
      }
...
void request_takeover(uint32_t now) {
  ...
  if (!torque_command_seen
      || now - last_torque_command_ms > bridge_cfg::TORQUE_COMMAND_TIMEOUT_MS) {
    emit_error("TORQUE_COMMAND_STALE", "A fresh psa_torque command is required before takeover");
    return;
  }
...
void handle_host_command(const char *line, uint32_t now) {
  ...
  if (strcmp(type, "psa_mads") == 0) {
    if (!doc["enabled"].is<bool>()) {
      emit_error("INVALID_MADS", "enabled must be a boolean");
      return;
    }
    const bool enabled = doc["enabled"].as<bool>();
    safety.on_mads_command(enabled, now);
    if (!enabled && state != GatewayState::Bypass) {
      restore_bypass("host disabled MADS");
    }
    JsonDocument ack;
    ack["type"] = "ack";
    ack["command"] = "psa_mads";
    ack["enabled"] = enabled;
    ack["state"] = safety.mads_state_name();
    ack["physical_enable"] = safety.physical_mads_enable();
    ack["rearm_required"] = safety.mads_rearm_required();
    if (safety.mads_reason()[0] != '\0') ack["reason"] = safety.mads_reason();
    emit_json(ack);
    return;
  }
  ...
  if (strcmp(type, "psa_torque") == 0) {
    ...
    torque_command_seen = true;
    last_torque_command_ms = now;
    JsonDocument ack;
    ack["type"] = "ack";
  ...
  if (strcmp(type, "psa_longitudinal") == 0) {
    ...
    if (!enabled) {
      ...
      const bool rvv_ready = state != GatewayState::Active || rvv.ready_for_takeover(now);
      if (state == GatewayState::Active
          && (!rvv_ready || !safety.longitudinal_inputs_ok(now))) {
        char reason[112];
        strlcpy(
            reason,
            !rvv_ready ? rvv.reason() : "RVV powertrain inputs do not permit control",
            sizeof(reason));
        rvv.set_command(false, psa_rvv_safety::MIN_SETPOINT_KPH, now);
        emit_error("LONGITUDINAL_NOT_READY", reason);
        return;
      }
    }
    JsonDocument ack;
...
void service_state(uint32_t now) {
  safety.tick(now);
  const bool rvv_fresh = !rvv.requested() || rvv.tick(now);
  if (rvv.requested() && (!rvv_fresh || !safety.longitudinal_inputs_ok(now))) {
    const char *reason = rvv_fresh
        ? "RVV powertrain input disengaged" : rvv.reason();
    emit_event("RVV_DISABLED_LATERAL_REMAINS", reason);
    rvv.set_command(false, psa_rvv_safety::MIN_SETPOINT_KPH, now);
  }
  ...
  if ((!torque_command_seen
    ...
    restore_bypass("host torque command timeout");
    return;
  }
  if (!safety.mads_engaged()
      && (state == GatewayState::Preparing || state == GatewayState::Settling
          || state == GatewayState::Active)) {
    restore_bypass(safety.mads_reason());
    return;
  }
...
void service_mads_physical_gate(uint32_t now) {
  const bool raw = digitalRead(bridge_cfg::MADS_ENABLE_PIN) == LOW;
  if (raw != mads_gate_raw) {
    mads_gate_raw = raw;
    mads_gate_changed_ms = now;
  }
  const bool enabled = raw
      && now - mads_gate_changed_ms >= bridge_cfg::MADS_PHYSICAL_DEBOUNCE_MS;
  if (enabled == mads_gate_enabled) return;
  mads_gate_enabled = enabled;
  safety.on_physical_mads_enable(enabled, now);
  emit_event(enabled ? "MADS_PHYSICAL_ENABLE" : "MADS_PHYSICAL_DISABLE");
}
...
void setup() {
  ...
  pinMode(bridge_cfg::MADS_ENABLE_PIN, INPUT_PULLUP);
  set_sbu1(false);
  safety.reset();
  rvv.reset();
  mads_gate_raw = digitalRead(bridge_cfg::MADS_ENABLE_PIN) == LOW;
  mads_gate_changed_ms = millis();
  safety.on_physical_mads_enable(false, mads_gate_changed_ms);
...
void loop() {
  ...
  service_mads_physical_gate(now);
  poll_interboard(now);
  poll_eps_can(now);
  service_can_alerts();
  poll_host(now);
