...
l'entrée de commande de l'étage SBU1 et `GPIO33` pour lire l'autorisation MADS
active-bas. Un interrupteur DPST place un pôle en série entre GPIO32 et `ON` du
TPS22919-Q1, et son second pôle entre GPIO33 et la masse. Le GPIO ne doit jamais
alimenter directement SBU1. Voir [MADS-T9](PSA_MADS_T9.md).
...
| Timeout de la commande torque | 150 ms |
...
Le RVV doit être sélectionné et déjà actif avant sa modification. L'accélérateur,
la disparition du RVV, le passage en LVV ou un timeout de sa commande arrêtent le
remplacement longitudinal et remettent immédiatement `0x50E` en transmission
stock, sans désengager MADS. Le frein reste un désengagement global conservateur.
Une donnée latérale périmée, une erreur UART ou un bus-off restaure le bypass.
...
| `psa-obdc-master-bench-torque` | Direction MADS sur banc | Oui, borné | Non | 30 s |
...
Les tests MADS purs et intégrés à la barrière LKA sont documentés dans
[`PSA_MADS_T9.md`](PSA_MADS_T9.md#validation).
...
Séquence d'armement MADS avec RVV de banc facultatif :

1. Fermer l'autorisation physique MADS.
2. Envoyer un heartbeat `engaged=true` au moins toutes les 100 ms.
3. Envoyer `psa_mads=true` et attendre `mads_state=enabled`.
4. Envoyer `psa_torque` au moins toutes les 100 ms, avec `raw=0` si nécessaire.
5. Activer le RVV stock et envoyer une cible seulement si le longitudinal est testé.
6. Demander la prise de contrôle.
7. Continuer les commandes périodiques nécessaires pendant la prise de contrôle.
8. Envoyer `psa_mads=false` ou `psa_takeover=false` pour revenir au bypass.
...
Cette dernière commande arrête seulement le remplacement RVV ; elle ne coupe
pas MADS ni le pont latéral.

Le retour `stats` expose notamment `rvv_stock_mode`, `rvv_stock_active`,
`rvv_stock_setpoint_kph`, `rvv_target_kph`, `rvv_applied_kph`,
`rvv_checksum_failures`, `rvv_controlled_frames`, `mads_state`,
`mads_rearm_required`, `mads_physical_enable` et `mads_reason`.
