...
def code_root(root: Path) -> Path:
  port = root / "openpilot/port/t9_lateral"
  port.mkdir(parents=True)
  (port / "base-sha256.json").write_text("{}\n")
  (port / "feature.py").write_text("VALUE = 15\n")
  return root
...
def test_export_selects_one_bounded_session_and_bundles_code(tmp_path):
  ...
  with tarfile.open(output) as archive:
    ...
    assert "code/openpilot/port/t9_lateral/feature.py" in names
