...
class Safety {
 public:
  ...
  void on_car_frame(uint32_t id, const uint8_t *data, uint8_t len, uint32_t now) {
    ...
    if (id == ID_STEERING && len >= 2) {
      ...
      if (abs_int(driver_torque_) > DRIVER_OVERRIDE_RAW) {
        disengage("driver torque override");
      }
      return;
    }
    ...
    if (id == ID_SPEED && len >= 6) {
      ...
      if (controls_allowed_ && !speed_in_envelope()) {
        disengage("speed outside configured control envelope");
      }
      return;
    }
    ...
    if (id == ID_DAT_BSI && len >= 1) {
      ...
      if (brake_pressed_) disengage("physical brake pressed");
      return;
    }
    ...
    if (id == ID_DYN_CMM && len >= 5) {
      ...
      if (!cruise_engaged_ || gas_pressed_) {
        disengage(gas_pressed_ ? "physical accelerator pressed" : "cruise inactive");
      } else if (!cruise_previous_ && heartbeat_engaged_ && inputs_fresh(now)
                 && stock_fresh(now) && speed_in_envelope()
                 && !brake_pressed_ && !relay_malfunction_) {
        controls_allowed_ = true;
        desired_torque_last_ = 0;
        rt_torque_last_ = 0;
        rt_torque_ms_ = now;
        set_reason("");
      }
      cruise_previous_ = cruise_engaged_;
    }
  }
  ...
  bool ready_for_takeover(uint32_t now) {
    ...
    if (!controls_allowed_) return reject("controls require a fresh cruise rising edge");
    if (!inputs_fresh(now)) return reject("safety CAN inputs stale or missing");
    if (!stock_fresh(now)) return reject("fresh usable stock 0x3F2 missing");
    if (brake_pressed_ || gas_pressed_ || !cruise_engaged_) return reject("pedal or cruise blocks takeover");
  ...
  void tick(uint32_t now) {
    ...
    if (!inputs_fresh(now)) disengage("safety CAN inputs stale or missing");
  }
  ...
  bool build_and_check_lka(int desired_torque, uint8_t *output, uint32_t now) {
    ...
    if (!bridge_cfg::ALLOW_NONZERO_TORQUE && desired_torque != 0) {
    ...
    if (abs_int(desired_torque) > MAX_TORQUE_RAW) return reject("torque exceeds +/-10 raw");

    const int delta = desired_torque - desired_torque_last_;
    const bool increasing = abs_int(desired_torque) > abs_int(desired_torque_last_);
    ...
    if (now - rt_torque_ms_ >= RT_INTERVAL_MS) {
      rt_torque_last_ = desired_torque;
      rt_torque_ms_ = now;
    } else if (abs_int(desired_torque - rt_torque_last_) > MAX_RT_DELTA) {
      return reject("real-time torque delta exceeds limit");
    }

    const uint16_t raw = static_cast<uint16_t>(desired_torque) & 0x7FFU;
    ...
    desired_torque_last_ = desired_torque;
    set_reason("");
    return true;
  }
  ...
  bool controls_allowed() const { return controls_allowed_; }
 ...
 private:
  ...
  bool inputs_fresh(uint32_t now) const {
    return fresh(driver_live_, now) && fresh(angle_live_, now) && fresh(speed_live_, now)
        && fresh(powertrain_live_, now) && fresh(brake_live_, now);
  }
  ...
  bool ready_during_takeover(uint32_t now) {
    tick(now);
    if (!relay_cut_ || !controls_allowed_ || !heartbeat_engaged_) return reject("controls not armed");
    if (!inputs_fresh(now)) return reject("safety CAN inputs stale or missing");
    if (brake_pressed_ || gas_pressed_ || !cruise_engaged_) return reject("pedal or cruise blocks steering");
