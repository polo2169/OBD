...
Cette passerelle correspond au harnais présent dans `PSA-Harness/` :
...
`ON` au GPIO 32, ajouter 100 kΩ de `ON` à GND et 100 nF près de `IN`. `QOD` peut
...
envoyer les heartbeats, puis provoquer un nouveau front montant du régulateur et
rester entre 50 et 90 km/h. Frein, accélérateur, couple conducteur, trame périmée,
CRC UART, bus-off ou fin des 400 ms remettent immédiatement `SBU1` à 0.
